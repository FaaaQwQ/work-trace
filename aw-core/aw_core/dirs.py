...
def get_data_dir(module_name: Optional[str] = None) -> str:
    data_dir = _worktrace_path("data") or platformdirs.user_data_dir(_get_appname())
    return os.path.join(data_dir, module_name) if module_name else data_dir


@_ensure_returned_path_exists
def get_cache_dir(module_name: Optional[str] = None) -> str:
    cache_dir = _worktrace_path("cache") or platformdirs.user_cache_dir(_get_appname())
...
def get_config_dir(module_name: Optional[str] = None) -> str:
    config_dir = _worktrace_path("config") or platformdirs.user_config_dir(_get_appname())
...
def get_log_dir(module_name: Optional[str] = None) -> str:  # pragma: no cover
    ...
    if _worktrace_path("logs"):
        log_dir = _worktrace_path("logs")
...
def _worktrace_path(kind):
    root = os.environ.get("WORKTRACE_HOME")
    if not root:
        return None
    profile = get_profile()
    if profile:
        root = os.path.join(root, "profiles", profile)
