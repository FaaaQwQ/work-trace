...
class AFKWatcher:
    ...
    def ping(self, afk: bool, timestamp: datetime, duration: float = 0):
        ...
        if recording["generation"]:
            boundary = datetime.fromtimestamp(float(recording["generation"]), timezone.utc)
            end = timestamp + timedelta(seconds=duration)
            timestamp = max(timestamp, boundary)
            duration = max(0, (end - timestamp).total_seconds())
            data["worktrace_session"] = recording["generation"]
