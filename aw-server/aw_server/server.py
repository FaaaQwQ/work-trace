...
class AWFlask(Flask):
    def __init__(
        ...
        if os.environ.get("WORKTRACE_HOME"):
            ...
            from .worktrace import Journal, blueprint

            self.journal = Journal(get_data_dir(), self.api)
            self.register_blueprint(blueprint(self.journal))
