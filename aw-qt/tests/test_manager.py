...
class TestModuleToggle:
    ...
    def test_worktrace_stop_terminates_windows_process_tree(self, module, monkeypatch):
        monkeypatch.setenv("WORKTRACE_HOME", "test-data")
        monkeypatch.setattr(manager_module.sys, "platform", "win32")
        monkeypatch.setattr(subprocess, "CREATE_NO_WINDOW", 0x08000000, raising=False)
        proc = MagicMock(spec=subprocess.Popen)
        proc.pid = 12345
        proc.returncode = None
        proc.wait.side_effect = lambda: setattr(proc, "returncode", 1)
