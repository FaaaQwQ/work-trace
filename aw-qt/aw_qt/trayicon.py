...
class TrayIcon(QSystemTrayIcon):
    ...
    def __init__(
        ...
        self.setToolTip(
            ("Work trace" if os.environ.get("WORKTRACE_HOME") else "ActivityWatch") + (f" ({profile})" if profile != DEFAULT_PROFILE else "")
        )
        ...
        self.worktrace_bar = None
        if os.environ.get("WORKTRACE_HOME"):
            from .worktrace import WorkTraceBar
            self.worktrace_bar = WorkTraceBar(self.root_url)
            self.worktrace_bar.reveal()
    ...
    def _build_rootmenu(self) -> None:
        ...
        if self.worktrace_bar:
            menu.addAction("Work trace 工作台", self.worktrace_bar.open_dashboard)
            menu.addAction("显示顶部悬浮栏", self.worktrace_bar.reveal)
            menu.addAction("打开工作记录文件夹", lambda: open_dir(os.environ["WORKTRACE_HOME"]))
            menu.addSeparator()
        ...
        if autostart.is_supported():
            self._autostart_action = menu.addAction(
                "登录后自动启动", self._on_autostart_toggled
            )
        ...
        if exitIcon.availableSizes():
            menu.addAction(exitIcon, "退出 Work trace" if self.worktrace_bar else "Quit ActivityWatch", lambda: exit(self.manager))
        else:
            menu.addAction("退出 Work trace" if self.worktrace_bar else "Quit ActivityWatch", lambda: exit(self.manager))
...
def run(
    ...
    if os.environ.get("WORKTRACE_HOME"):
        from .worktrace import brand_icon, apply_windows_style
        icon = brand_icon()
