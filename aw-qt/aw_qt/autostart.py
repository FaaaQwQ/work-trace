...
APP_NAME = "Work trace" if os.environ.get("WORKTRACE_HOME") else "ActivityWatch"
...
def _command() -> List[str]:
    ...
    if os.environ.get("WORKTRACE_LAUNCHER"):
        python = Path(sys.executable)
        if sys.platform == "win32" and python.with_name("pythonw.exe").exists():
            python = python.with_name("pythonw.exe")
        command = [str(python), os.environ["WORKTRACE_LAUNCHER"], "--data-dir", os.environ["WORKTRACE_HOME"]]
        if _profile() == "testing":
